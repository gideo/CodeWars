// 6 kyu - Reducing by rules to get the result
...
// You have to use these rules consecutively. So when you get to the end 
// of the list of rules, you start again at the beginning.
...
// There are two rules. First rule says: Sum the two numbers a and b. 
...
function reduceByRules(numbers, rules){
  let result = numbers[0], idx = 0;
  for(let i = 1; i < numbers.length; i+=1){
    result = rules[idx](result, numbers[i]);
    idx++;
    if(idx > rules.length-1) { idx = 0; }
  }
  return result;
}