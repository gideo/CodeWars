...
// Write a function sumTimesTables which sums the result of the sums of the elements 
// specified in tables multiplied by all the numbers in between min and max including 
// themselves.
...
function sumTimesTables(tables, min, max){
  let sum = (max * (max + 1) / 2 - (min * (min - 1)/2));
  return tables.reduce((a,b) => a + (b * sum),0);
}