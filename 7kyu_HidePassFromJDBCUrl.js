...
// We have to create a function that receives a connection string with 
// password included and you have to mask the password i.e. change 
// password by asterisks.
...
// empty passwords are not validated but best solutions take empty passwords 
// into account
...
function hidePasswordFromConnection(urlString) {
  return urlString.replace(/password=([^&]*)/, (u, p) => `password=${'*'.repeat(p.length)}`);
}