...
function toTable(data, headers=false, index=false) {
  data = data.map((v, i) => {
    if (index) {
      let ic = headers ? i : i + 1;
      v.unshift( i === 0 && headers ? '' : ic  );
    }
    return headers && i === 0 ? '<tr><th>' + v.join('</th><th>') + '</th></tr>' : '<tr><td>' + v.join('</td><td>') + '</td></tr>';
  });
  return "<table>" + (headers  ? '<thead>' + data.shift()  + '</thead>' : '' ) + '<tbody>' + data.join('') + '</tbody>'+ "</table>";
}