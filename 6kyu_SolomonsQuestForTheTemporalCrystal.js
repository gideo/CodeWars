// 6kyu - Solomon's Quest for the Temporal Crystal
...
function solomonsQuest(arr){
  let pos = [0,0];
  for(let i=0, d=1; i<arr.length; i++) {
    d *= Math.pow(2,arr[i][0]);
    arr[i][1]<2?pos[arr[i][1]]+=d*arr[i][2]:pos[arr[i][1]-2]-=d*arr[i][2];
  }
  return [pos[1],pos[0]];
}