// 6kyu - Which filetypes are you using the most?
...
function solve(files) {
  let obj = files.map(file => file.match(/\.\w+$/)[0]).reduce((a,c) => (a[c] = (a[c]||0) + 1, a), {}),
      max = Math.max(...Object.values(obj));
  return Object.keys(obj).sort().filter(key => obj[key] == max);
}