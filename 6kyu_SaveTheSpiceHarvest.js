...
// Goal:
// As cda spotter pilot, you are responsible for handling dispatch of Carryalls in your vicinity. Your goal is to determine whether a carryall should be sent for rescue, or if it must be forfeited because there is not enough time.
...
function harvesterRescue(data) {
  let a = Math.sqrt((data.worm[0][0] - data.harvester[0]) ** 2 + (data.worm[0][1] - data.harvester[1]) ** 2),
      b = Math.sqrt((data.carryall[0][0] - data.harvester[0]) ** 2 + (data.carryall[0][1] - data.harvester[1]) ** 2);
  return a / data.worm[1] - b / data.carryall[1] > 1 ? "The spice must flow! Rescue the harvester!" : "Damn the spice! I'll rescue the miners!";
}