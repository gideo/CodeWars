...
class Potion {
  ...
  mix(other) {
    let r = Math.ceil( (other.color[0]*other.volume+this.color[0]*this.volume)/(other.volume+this.volume) ),
        g = Math.ceil( (other.color[1]*other.volume+this.color[1]*this.volume)/(other.volume+this.volume) ),
        b = Math.ceil( (other.color[2]*other.volume+this.color[2]*this.volume)/(other.volume+this.volume) );
    return new Potion([r,g,b],other.volume+this.volume);    
    }
}