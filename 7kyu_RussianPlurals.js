// 7kyu - Russian plurals

// There is an easy Kata about plurals.

// You aren't able to use any string/array/object methods, libs etc. You have to 
// use only couple of math operators and conditionals. "+" and "-" operators also 
// disabled.
...
// You need only 2 options and pretty easy logic in your function. Every number 
// is plural except of 1.
...
// Ok, there were a couple of examples, which needs only to scare you. Russian 
// rule is easy too:

// Number endings: 1 (example: 1, 21, 41, 121) (but not 11, 111 etc.) — singular.
// Number endings: 2,3,4 (example: 22, 42, 123) (but not 12, 13 & 14) — genitive 
// singular.
// Number endings: 5,6,7,8,9,0 (includes 11, 12, 13, 14) — genitive plural.
// Write the function, which have the array of option and the number. It should 
// return string with correct option.

// * There is an array ['день', 'дня', 'дней'] in test cases for easier debugging

const makePlural = (options, number) => {
  number %= 100; 
  if( number % 10 == 1 && number !== 11 ) return options[0];
  return  ((number % 10 == 2 || number % 10 == 3 || number % 10 == 4) && 
          (number !== 12 && number !== 13 && number !== 14)) ? options[1] : options[2];
}